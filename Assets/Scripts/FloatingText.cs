...
public class FloatingText : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (attached == AttachPoint.None) {
			...
			this.gameObject.GetComponent<Renderer> ().enabled = true;
		} else if (attached == AttachPoint.LeftHandMagazine || attached == AttachPoint.RightHandMagazine) {
			this.GetComponent<TextMesh> ().text = "Press the secondary button to load the magazine.";
			this.gameObject.GetComponent<Renderer> ().enabled = true;
		} 
		...
		if (this.gameObject.GetComponent<Renderer> ().enabled) {
			...
			} else if (attached == AttachPoint.LeftHandMagazine) {
				snapTransform = GameObject.FindGameObjectWithTag ("LeftHand").transform;
			} else if (attached == AttachPoint.RightHandMagazine) {
				snapTransform = GameObject.FindGameObjectWithTag ("RightHand").transform;
			}  
